...
\renewcommand{\arraystretch}{1.50}
    ...
    \begin{itemize}
        ...
        \item kako bi se omogućilo korištenje sistemskog \textit{clipboard}-a, instaliran je \texttt{xclip}
    \end{itemize}
    \begin{tabular}{|>{\tt}p{9.00cm}|>{}p{15.50cm}|}
        \hline
        ctrl+b,[                                    &   ulazak u \textit{copy mode}                                                                                         \\ \hline
        <space>                                     &   početak označavanja teksta                                                                                          \\ \hline
        <esc>                                       &   prekid označavanja teksta                                                                                           \\ \hline
        q                                           &   odustajanje / izlazak iz \textit{copy mode}-a                                                                       \\ \hline
        y                                           &   kraj označavanja teksta (\textit{copy} u sistemski \textit{clipboard}) i izlazak iz \textit{copy mode}-a            \\ \hline
        <enter>                                     &   kraj označavanja teksta (\textit{copy} u \texttt{tmux}-ov \textit{clipboard}) i izlazak iz \textit{copy mode}-a     \\ \hline
        ctrl+b,]                                    &   \textit{paste} iz \texttt{tmux}-ovog \texttt{clipboard}-a                                                           \\ \hline
    \end{tabular}

    % Customization
    \section*{\color{ForestGreen} Customization \texttt{$\sim$/.tmux.conf}}
    \texttt{
        \scriptsize
